...
// CODE HERE
// function helloOut(event) {
//     event.target.classList.remove('buttonChange');
// }

// sayHelloButton.addEventListener('mouseout', helloOut);

sayHelloButton.addEventListener('mouseout', () => {
    sayHelloButton.classList.remove('buttonChange');
})
...
*/ 
    ...
    /// can create the p in the div, but only once, and only the last array item
const ohMy = () => {
    axios.get('http://localhost:3000/animals').then((res) => {
        const para = document.createElement("p");
        const poopy = document.querySelector('#thediv'); 
        
        for (let i = 0; i < res.data.length; i++) {
            console.log(res.data[i]);
            para.textContent = res.data[i];
            poopy.appendChild(para);
        }
    })
}
...
const repeatMyParam = () => {
    ...
    axios.get('http://localhost:3000/repeat/Nathaneal-is-a-drunk').then((res) => {
...
function newFunction () {
    axios.get('http://localhost:3000/query-test/?name=peyton+moss').then((res) => {
        console.log(res.data)
    })
}

document.querySelector('#query-button').addEventListener('click', newFunction);
